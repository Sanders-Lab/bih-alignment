...
args = c('counts.txt.gz', 'counts.info' ,'counts.pdf')
...
rhino_chrom_convert = function(df, og_name = 'chrom', filter_na = T){
    chrom_convert = read.table('/data/cephfs-1/work/projects/sanders-nwr/references/david-assemblies/chrom_convert_key.tsv',
                               header = T) 
    if(og_name != 'chrom'){
        chrom_convert = select(chrom_convert, !!og_name := chrom, chrom_use)
    }
    df_proc = df %>% 
        left_join(chrom_convert) %>% 
        select(-c(!!as.name(og_name)),
               !!og_name := chrom_use) 
    if(filter_na) df_proc = filter(df_proc, !is.na(!!as.name(og_name)))
    if(which(names(df) == og_name)==1) df_proc = dplyr::select(df_proc, !!og_name, everything())
    chrom_order = paste0(rep(paste0('chr',c(1:40,'X')),each = 7),c('',paste0('.',1:6)))
    df_proc = mutate(df_proc, !!og_name := droplevels(factor(!!as.name(og_name), levels = chrom_order, ordered = T)))
    return(df_proc)
}
...
d = fread(f_in)
# d = fread('/data/cephfs-1/work/projects/sanders-nwr/scratch/20250728-david-ass-aln/suni-scaffold-h2/qc/mosaicatcher/counts.txt.gz')
...
# # Re-name and -order chromosomes - this is rhino-specific
# length_input_chroms = length(unique(d$chrom[grepl('^CM',d$chrom)]))
# message('no of chroms: ',length_input_chroms)
# if(length_input_chroms==41){
#     chromstouse = paste0('chr',c(1:40,'X'))
# }else if(length_input_chroms==42){
#     chromstouse = paste0('chr',c(1:40,'X','M'))
# }else if(length_input_chroms == 44 & 'CM038192.1_RagTag.3' %in% d$chrom){
#     message('special case for suni-scaffold-h2, chrX in 3 contigs')
#     chromstouse = paste0('chr',c(1:40,'X_1','X_2','X_3','M'))
# }else{
#     stop('the number of chromosomes starting with CM should be 41 (autosomes + X) or 42 (autosomes + X + M), currently it is: ',
#          length_input_chroms)
# }

# chrom_convert = data.frame('chrom' = unique(d$chrom[grepl('^CM',d$chrom)]),
#            'chrom_use' = chromstouse)
# 
# og_count = count(d,chrom, name = 'n_og')
# d = d %>% 
#     left_join(chrom_convert) %>% 
#     filter(!is.na(chrom_use),
#            chrom_use != 'chrM') %>% 
#     select(-c(chrom),
#            chrom = chrom_use)

d = rhino_chrom_convert(d)

table(d$chrom)
new_count = count(d,chrom, name = 'n_new') %>% 
    rename(chrom_new = chrom)

# test_df = cbind(new_count[match( chromstouse[chromstouse!='chrM'], new_count$chrom),],
#       head(og_count,length(chromstouse[chromstouse!='chrM']))) %>% 
#     mutate(same_query = ifelse(n_new == n_og,T,F))
# 
# if(!all(test_df$same_query)) stop('error converting chrom names')
chromstouse = unique(d$chrom)
d = d[, chrom := sub('^chr','',chrom)]
# d = d[grepl('^([1-9]|[12][0-9]|X|Y)$', chrom),]
d = d[, chrom := factor(chrom, levels=sub('^chr','',chromstouse), ordered = T)]
