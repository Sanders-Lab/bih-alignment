...
for library in $libraries; do
	...
	# No. of reads in BAM
	n_reads_unfilt=$(samtools view -c -@ 4 -f 64 $bamfile) # count only first read in pair
 	[ -z "$n_reads_unfilt" ] && n_reads_unfilt="NA"
	# unique reads that are mapped
 	n_reads_good=$(samtools view -c -@ 4 -f 66 -F 3200 -q 10 $bamfile) # take only unique, first in pair reads that are mapped in proper pair
	...
	# [ -z "$n_reads_mapped" ] && n_reads_mapped="NA"
 	# No. duplicated reads
  	n_reads_dup=$(samtools view -@ 4 -f 1090 $bamfile | wc -l)
   	...
   	# Duplication rate
    	dupl_rate=$(echo $n_reads_dup / $n_reads_unfilt | bc -l | head -c4)
	...
	# save output to file
	echo $library $gc_content $n_reads_unfilt $n_reads_good $n_reads_dup  $dupl_rate $mean_insert $complexity $coverage $meandp | tr " " "\t" >> ${statsdir}/all_samples_qc_metrics.txt
	) &
	if [[ $(jobs -r -p | wc -l) -ge $n_threads_divided ]]; # allows n_threads number of jobs to be executed in parallel
   	then
       	wait -n
